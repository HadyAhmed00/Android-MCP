...
**Package layout:** everything lives in `android_mcp/`. Root `main.py` is a 3-line shim kept so existing MCP configs pointing at an absolute `main.py` path keep working; the published console script is `android-mcp = android_mcp.server:main`.
