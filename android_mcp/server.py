...
from android_mcp.recorder import TestRecorder
...
# Populated by main(); tools resolve these at call time, not import time.
...
def _build_parser() -> ArgumentParser:
    parser = ArgumentParser(prog='android-mcp', description='MCP server for Android devices')
    parser.add_argument('command', nargs='?', default='serve', choices=['serve', 'setup'],
                        help="'serve' (default) runs the MCP server; 'setup' installs the Portal app and exits")
    parser.add_argument('--emulator', action='store_true', help='Use the emulator (emulator-5554)')
    parser.add_argument('--device', type=str, default=None,
                        help='Specific device ID (e.g., b44fbcc9 or emulator-5554)')
    parser.add_argument('--skip-bootstrap', action='store_true',
                        help='Never auto-install the Portal app on the device')
    parser.add_argument('--portal-apk', type=str, default=None,
                        help='Install this local Portal APK instead of downloading the pinned one')
    parser.add_argument('--force-setup', action='store_true',
                        help='setup only: reinstall the Portal app even if it is already working')
    return parser


def _resolve_device(args) -> str | None:
    """Device precedence: --device > --emulator > None (auto-detect)."""
    if args.device:
        return args.device
    if args.emulator:
...
def main() -> None:
    global device_id, mobile, recorder, _adb

    args = _build_parser().parse_args()
    device_id = _resolve_device(args)
    _adb = Adb(device_id)

    if args.command == 'setup':
        sys.stderr = _original_stderr
        report = ensure_portal(device_id, force=args.force_setup, apk_path=args.portal_apk)
    ...
    mobile = Mobile(
        device=device_id,
        use_mcp_helper=True,
        auto_bootstrap=not args.skip_bootstrap,
        portal_apk=args.portal_apk,
    )
...
@mcp.tool(name='Setup-Device',description='Install the Portal helper app on the device and enable its accessibility service. Use this when State-Tool is slow or the fast MCP Helper backend is unavailable.')
def setup_device_tool(force:bool=False):
    report = ensure_portal(device_id, force=force)
    if report.ok and mobile is not None:
        # Let the next state read re-attempt the fast path.
        mobile._mcp_init_attempted = False
        mobile.use_mcp_helper = True
...
def report_bug_to_azure_command(
    ...
    description_steps += "<p><strong>Steps to Reproduce:</strong></p>"
...
def report_bug_to_azure(
    ...
    try:
        ...
        success_msg = "✅ Bug reported successfully to Azure DevOps!\n\n"
        ...
        success_msg += "   ✓ Steps to Reproduce\n"
        success_msg += "   ✓ Expected Result\n"
        success_msg += "   ✓ Actual Result\n"
        success_msg += "   ✓ Environment Info\n"
        if bug_url:
