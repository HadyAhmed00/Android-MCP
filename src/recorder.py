...
class TestRecorder:
    ...
    def _export_as_adb_python(self, filepath: Path, test_name: str) -> str:
        ...
        script_lines = [
            ...
            '        import re',
            '        if not text:',
            '            self.click(x_fallback, y_fallback)',
            '            return',
            '        try:',
            ...
            '                h_m = re.search(r\'hint="([^"]*)"\', node)',
            '                found = ((t_m and text.lower() in t_m.group(1).lower()) or',
            '                         (cd_m and text.lower() in cd_m.group(1).lower()) or',
            '                         (h_m and text.lower() in h_m.group(1).lower()))',
            ...
            '        import re',
            '        if not text:',
            '            self.long_click(x_fallback, y_fallback)',
            '            return',
            '        try:',
            '            subprocess.run(f"adb -s {self.device_id} shell uiautomator dump /sdcard/_d.xml",',
            '                           shell=True, capture_output=True, timeout=8)',
